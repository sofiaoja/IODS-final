...
- Data was loaded for further analysis using correlation, linear regression,  and dimensionality reduction techniques and results were interpreted.
...
```
Normal distributions and correlations between the parameters were studied. 
...
# The relationship between the chosen explanatory variables and the target 
# variable is statistically significant for variable p1_006$ObjectLengthCh1 and p1_006$ObjectWidthCh1 ('***' 0.001), and 
# for p1_006$ObjectPerimCh1 ('*' 0.05). Multiple R squared of the model is 0.8307, which is 
# quite high thus linear model might explain this data. (Ref Wikipedia: In statistics, the coefficient of multiple correlation is a measure of how well a given variable can be predicted using a linear function of a set of other variables).
...
# Some outliers seen from diagnostic plots, e.g. in Normal Q-Q plot data not linear in the beginnig and at the end points.
#par(mfrow = c(2,2))
```
Data was further analysed using linear regression model. The relationship between the chosen explanatory variables and the target variable is statistically significant for variable ObjectLengthCh1 and ObjectWidthCh1 (0.001), and for ObjectPerimCh1 (0.05). Multiple R squared of the model is 0.8307, which is high thus linear model explains well this data.
```{r}
...
```
Data was further analysed using dimensionality reduction techniques, principal component analysis (PCA). PC1 explains 47.1%  and PC2 14.6% of the variance.
```{r}
# draw a biplot
